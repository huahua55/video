    ...
    public function data()
    {
        ...
        if(!empty($param['repeat'])){
            if($param['page'] ==1){
                ...
                Db::execute('CREATE TABLE IF NOT EXISTS `'.config('database.prefix').'tmpart` (`id1` int(10) unsigned, `name1` varchar(255) NOT NULL DEFAULT \'\')');
                Db::execute('insert into `'.config('database.prefix').'tmpvod` (SELECT min(art_id),art_name FROM '.config('database.prefix').'art GROUP BY art_name HAVING COUNT(art_name)>1)');
            }
            $order='art_name asc';
