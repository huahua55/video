...
class videoVod extends Base {
    ...
    public function listData($whereOr =[],$where,$order,$page=1,$limit=20,$start=0)
    {
        ...
        log::write('sql结束-'.Db::table('video_vod')->getLastSql().'-'.msectime());
...
//        p(1);
        log::write('sql开始-'.Db::table('video_collection')->getLastSql().'-'.msectime());
        $collection_list =  Db::table('video_collection')->where($where_collection)->column(null,'task_id');
        log::write('sql结束-'.Db::table('video_collection')->getLastSql().'-'.msectime());
        foreach ($list as $k=>$v){
            $list[$k]['examine_txt'] = '';
            $list[$k]['mu_url'] = '';
