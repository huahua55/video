...
mysql_config = {'host': '45.120.216.57', 'port': '6306', 'user': 'uservide', 'passwd': 'C@HjduTY793CKLp',
                'dbname': 'video',
                'charset': 'utf8mb4'}
...
# 总数量
total_count = sql_count("select count(id) as total_count from video where tx_vod_id != 0 and vod_total != 1")
...
while total_page > 0:
    ...
    #         page_limit) + ",20"
    video_sql = "select `id`,`tx_vod_id`, `type_pid`, `type_id`, `vod_name`, `vod_sub`, `vod_en`, `vod_tag`, `vod_pic`, `vod_pic_thumb`, `vod_pic_slide`, `vod_actor`, `vod_director`, `vod_writer`, `vod_behind`, `vod_blurb`, `vod_remarks`, `vod_pubdate`, `vod_total`, `vod_serial`, `vod_tv`, `vod_weekday`, `vod_area`, `vod_lang`, `vod_year`, `vod_version`, `e_id`, `vod_state`, `vod_duration`, `vod_isend`, `vod_douban_id`, `vod_douban_score`, `vod_time`, `vod_time_add`, `is_from`, `is_examine`, `vod_status`, `vod_id`, `is_selected` from video where  tx_vod_id != 0 and vod_total != 1"
    ...
    if result is not None:
        for index_key, index_val in enumerate(result):
            ...
            where = 'vod_name =  "' + vod_name + '" and type_pid =  "' + str(
                type_pid) + '"  and vod_mango_albumId is Null '
            whereNo = ""
            wherei = 0
            tx_vod_sql = ""
            try:
                tx_vod_sql_q = "select * from tx_vod where id = " + str(
                    index_val['tx_vod_id']) + "  and vod_mango_albumId is Null"
                result_tx_vod = db1.fetchall(tx_vod_sql_q)
                # 是否 不存在
                if len(result_tx_vod) == 0:
                    ...
                    if vod_director:
                        wherei = wherei + 1
                        whereNo = common_where(vod_director, whereNo, where_name='vod_director', type=' or ',
                                               wherei=wherei)
                    # sql
                    if whereNo != "":
                ...
                if len(result_tx_vod) == 0:
                    ...
                    if len(result_tx_vod) > 1:
                        # p(result_tx_vod)
                        common_print_log("    --tx_vod找到数据多 过滤：" + str(index_val['id']) + '；名称：' + str(
                    ...
                    if int(tx_vod_array['vod_total']) > 1:
                        print('>>>>>>>>>>>>>>>>>>>>' + str(tx_vod_array['vod_total']))
                        up_data = {}
                        # 判断 tx_vod 是否存在此数据
                        up_list_key = [
                            'vod_sub',
