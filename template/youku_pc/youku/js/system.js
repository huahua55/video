...
var MAC = {
	...
	'Copy': function(s) {
		if (window.clipboardData) {
			...
			$('#mac_flash_copy').html('<embed src=' + SitePath + '"images/_clipboard.swf" FlashVars="clipboard=' + escape(s) +
	...
	'Open': function(u, w, h) {
		window.open(u, 'macopen1', 'toolbars=0, scrollbars=0, location=0, statusbars=0,menubars=0,resizable=yes,width=' + w +
			',height=' + h + '');
	},
	'Cookie': {
	...
	'Hits': {
		'Init': function() {
			...
			$.ajax({
				type: 'get',
				url: maccms.path + '/index.php/ajax/hits?mid=' + $that.attr("data-mid") + '&id=' + $that.attr("data-id") +
					'&type=update',
				timeout: 5000,
				dataType: 'json',
				error: function() {
	...
	'Score': {
		'Init': function() {
			...
			$.ajax({
				type: 'post',
				url: maccms.path + '/index.php/ajax/score?mid=' + $('.mac_score').attr('data-mid') + '&id=' + $('.mac_score').attr(
					'data-id'),
				timeout: 5000,
				error: function() {
		...
		'Init': function() {
			...
			$('.mac_star').raty({
				...
				click: function(score, evt) {
					$.ajax({
						type: 'get',
						url: maccms.path + '/index.php/ajax/score?mid=' + $('.mac_star').attr('data-mid') + '&id=' + $('.mac_star')
							.attr('data-id') + '&score=' + (score * 2),
						timeout: 5000,
						dataType: 'json',
						error: function() {
		...
		'Init': function() {
			$('body').on('click', '.digg_link', function(e) {
				...
				if ($that.attr("data-id")) {
					$.ajax({
						url: maccms.path + '/index.php/ajax/digg.html?mid=' + $that.attr("data-mid") + '&id=' + $that.attr(
							"data-id") + '&type=' + $that.attr("data-type"),
						cache: false,
						dataType: 'json',
						success: function($r) {
	...
	'Gbook': {
		...
		'Report': function(name) {
			MAC.Pop.Show(400, 300, '数据报错', maccms.path + '/index.php/gbook/report.html?name=' + encodeURIComponent(name),
				function($r) {
	...
	'History': {
		...
		'Init': function() {
			...
			html +=
				'<dd></dd><dd><a href="javascript:void(0)" onclick="MAC.History.Clear();">清空历史记录</a><span>历史记录</span></dd><dt></dt></i></dl></div></div></div></div>';
	...
	'Ulog': {
		...
		'Set': function() {
			if ($(".mac_ulog_set").attr('data-mid')) {
				...
				$.get(maccms.path + '/index.php/user/ajax_ulog/?ac=set&mid=' + $that.attr("data-mid") + '&id=' + $that.attr(
					"data-id") + '&sid=' + $that.attr("data-sid") + '&nid=' + $that.attr("data-nid") + '&type=' + $that.attr(
					"data-type"));
			}
		},
		'Click': function() {
			$('body').on('click', 'a.mac_ulog', function(e) {
				...
				if ($that.attr("data-id")) {
					$.ajax({
						url: maccms.path + '/index.php/user/ajax_ulog/?ac=set&mid=' + $that.attr("data-mid") + '&id=' + $that.attr(
							"data-id") + '&type=' + $that.attr("data-type"),
						cache: false,
						dataType: 'json',
						success: function($r) {
		...
		'BuyPopedom': function(o) {
			...
			if ($that.attr("data-id")) {
				if (confirm('您确认购买此条数据播放权限吗？')) {
					$.ajax({
						url: maccms.path + '/index.php/user/ajax_buy_popedom.html?id=' + $that.attr("data-id") + '&sid=' + $that.attr(
							"data-sid") + '&nid=' + $that.attr("data-nid") + '&type=' + $that.attr("data-type"),
						cache: false,
						dataType: 'json',
						success: function($r) {
							$that.addClass('disabled');
	...
	'Comment': {
		...
		'Show': function($page) {
			if ($(".mac_comment").length > 0) {
				$.ajax({
					type: 'get',
					url: maccms.path + '/index.php/comment/ajax.html?rid=' + $('.mac_comment').attr('data-id') + '&mid=' + $(
						'.mac_comment').attr('data-mid') + '&page=' + $page,
					timeout: 5000,
					error: function() {
		...
		'Submit': function($o) {
			...
			$.ajax({
				...
				data: $(form).serialize() + '&comment_mid=' + $('.mac_comment').attr('data-mid') + '&comment_rid=' + $(
